...
public interface HomeMapper {
    ...
    /** 进行中任务数 */
    @Select("SELECT COUNT(*) FROM task WHERE is_template = 0 AND task_start_time <= NOW() AND task_end_time >= NOW()")
    long countCurrentTasksForTeacher();

    /** 强制任务数 */
    @Select("SELECT COUNT(*) FROM task WHERE is_template = 0 AND is_mandatory = 1")
    long countForceTasksForTeacher();

    /** 总任务数 */
    @Select("SELECT COUNT(*) FROM task WHERE is_template = 0")
    long countTotalTasksForTeacher();

    /** 完成统计（基于 task_user） */
    @Select("SELECT COUNT(*) AS total, " +
            "SUM(CASE WHEN tu.status = '1' THEN 1 ELSE 0 END) AS completed, " +
            "SUM(CASE WHEN tu.status = '0' THEN 1 ELSE 0 END) AS uncompleted " +
            "FROM task_user tu JOIN task t ON t.id = tu.task_id WHERE t.is_template = 0")
    ...
    @Select("SELECT t.id AS task_id, t.task_name, t.task_end_time AS end_time, " +
            ...
            "ORDER BY (SUM(CASE WHEN tu.status = '1' THEN 1 ELSE 0 END) * 1.0 / COUNT(*)) ASC LIMIT 3")
    List<Map<String, Object>> lowCompletedTop3ForTeacher();

    // ==================== headmaster 范围（create_by） ====================

    @Select("SELECT COUNT(*) FROM task WHERE is_template = 0 AND task_start_time <= NOW() AND task_end_time >= NOW() AND create_by = #{createBy}")
    long countCurrentTasksForHeadmaster(@Param("createBy") Long createBy);

    @Select("SELECT COUNT(*) FROM task WHERE is_template = 0 AND is_mandatory = 1 AND create_by = #{createBy}")
    long countForceTasksForHeadmaster(@Param("createBy") Long createBy);

    @Select("SELECT COUNT(*) FROM task WHERE is_template = 0 AND create_by = #{createBy}")
    long countTotalTasksForHeadmaster(@Param("createBy") Long createBy);

    @Select("SELECT COUNT(*) AS total, " +
            "SUM(CASE WHEN tu.status = '1' THEN 1 ELSE 0 END) AS completed, " +
            "SUM(CASE WHEN tu.status = '0' THEN 1 ELSE 0 END) AS uncompleted " +
            "FROM task_user tu JOIN task t ON t.id = tu.task_id WHERE t.is_template = 0 AND t.create_by = #{createBy}")
    ...
    @Select("SELECT t.id AS task_id, t.task_name, t.task_end_time AS end_time, " +
            ...
            "SUM(CASE WHEN tu.status = '0' THEN 1 ELSE 0 END) AS uncompleted " +
            "FROM task t JOIN task_user tu ON tu.task_id = t.id " +
            "WHERE t.is_template = 0 AND t.create_by = #{createBy} " +
            "GROUP BY t.id, t.task_name, t.task_end_time " +
            "ORDER BY (SUM(CASE WHEN tu.status = '1' THEN 1 ELSE 0 END) * 1.0 / COUNT(*)) ASC LIMIT 3")
    ...
    /** 已完成任务数 */
    @Select("SELECT COUNT(*) FROM task_user WHERE user_id = #{userId} AND status = '1'")
