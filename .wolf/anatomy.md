...
- `BatchTaskService.java` — 批处理任务服务 —— 定时任务的共享逻辑。 (~1834 tok)
...
- `OpenApiConfig.java` — OpenAPI 配置 —— 声明 JWT Bearer Token 认证方式与标签展示顺序。 (~574 tok)
...
- `SysUserRepository.java` — 系统用户仓储接口 —— 定义 sys_user 的数据访问契约。 (~456 tok)
...
- `NotificationMessageEntity.java` — 通知消息持久化实体，映射 notification_message 表。 (~592 tok)
...
- `SysUserRepositoryImpl.java` — 系统用户仓储实现 —— 使用 MyBatis-Plus 操作 MySQL。 (~2203 tok)
